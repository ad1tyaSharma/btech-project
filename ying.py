# -*- coding: utf-8 -*-
"""
Created on Wed Apr 19 22:40:30 2023
...
def maxEntropyEnhance(I, isBad, a=-0.3293, b=1.1258):
    ...
    isBad = isBad * 1
    isBad = np.array(Image.fromarray(isBad).resize(size = (50,50)))
    isBad[isBad<0.5] = 0
    isBad[isBad>=0.5] = 1
    Y = Y[isBad==1]
...
def Ying(img, mu=0.5, a=-0.3293, b=1.1258):
    lamda = 0.5
    sigma = 5
    I = cv2.normalize(img.astype('float64'), None, 0.0, 1.0, cv2.NORM_MINMAX)
    ...
    t_our = cv2.resize(tsmooth(np.array(im.resize(size)), lamda, sigma), (t_b.shape[1], t_b.shape[0]), interpolation=cv2.INTER_AREA)
